import torch
import numpy as np
import pandas as pd
import datetime as dt
import yfinance as yf
import matplotlib.pyplot as plt
import matplotlib.dates as mdates
import textwrap
from data_pipeline import marketdatacollector
from models.tft import temporalfusiontransformer


class liveforecaster:
    def __init__(self, model_path: str = "checkpoints/best_tft_model.pt"):
        self.device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
        self.model = temporalfusiontransformer(
            static_input_size=1,
            past_input_sizes={"Close": 1, "Volume": 1, "rsi_14": 1},
            future_input_sizes={"day_of_week": 1, "month": 1},
            hidden_size=64
        ).to(self.device)
        self.model.load_state_dict(torch.load(model_path, map_location=self.device))
        self.model.eval()

    def analyze_and_plot(self, ticker: str, static, past, future, financials, recent_data, purchase_price, past_dates,
                         future_dates, hist_vol, vol_min, vol_max):
        # push tensors to apple silicon to match the model location
        static = static.to(self.device).contiguous()
        past = past.to(self.device).contiguous()
        future = future.to(self.device).contiguous()

        # forward pass
        with torch.no_grad():
            quantiles_output = np.sort(self.model(static, past, future).squeeze(0).cpu().numpy(), axis=-1)

        # un-scale predictions back to raw decimals, then multiply by 100 for true percentage
        p10_raw = quantiles_output[:, 0] * (vol_max - vol_min) + vol_min
        p50_raw = quantiles_output[:, 1] * (vol_max - vol_min) + vol_min
        p90_raw = quantiles_output[:, 2] * (vol_max - vol_min) + vol_min

        p10 = p10_raw * 100
        p50 = p50_raw * 100
        p90 = p90_raw * 100

        # extract metrics
        current_price = financials.get('currentPrice', recent_data['Close'].iloc[-1])
        target_price = financials.get('targetMeanPrice', current_price)
        pe_ratio = financials.get('trailingPE', 'N/A')
        current_rsi = recent_data['rsi_14'].iloc[-1]

        # calculate p/l if purchase price is provided
        if purchase_price is not None:
            pnl_pct = ((current_price - purchase_price) / purchase_price) * 100
            pnl_str = f"{pnl_pct:+.2f}%"
            avg_cost_str = f"${purchase_price:.2f}"
            position_context = f"POSITION {pnl_str}. "
        else:
            pnl_str = "N/A"
            avg_cost_str = "N/A"
            position_context = ""

        # stark terminal signal logic
        signal, color = "HOLD", "#f39c12"
        reasoning = f"{position_context}NEUTRAL MOMENTUM. TRADING NEAR CONSENSUS FAIR VALUE."

        if target_price > (current_price * 1.05) and current_rsi < 60:
            signal, color = "BUY", "#00e676"
            reasoning = f"{position_context}DISCOUNT TO CONSENSUS > 5%. RSI INDICATES HEALTHY MOMENTUM."
        elif current_rsi >= 70:
            signal, color = "SELL (IF HELD)", "#ff1744"
            reasoning = f"{position_context}RSI OVERBOUGHT ({current_rsi:.1f} >= 70). HIGH PROBABILITY OF CONSOLIDATION."
        elif current_price > target_price:
            signal, color = "SELL (IF HELD)", "#ff1744"
            reasoning = f"{position_context}PRICE EXCEEDS CONSENSUS TARGET. MATHEMATICAL UPSIDE CAPPED."

        # plotting - true black terminal aesthetic
        plt.style.use('dark_background')
        plt.rcParams.update({'font.family': 'monospace'})

        fig, ax = plt.subplots(figsize=(14, 8), dpi=300)
        fig.patch.set_facecolor('#000000')
        ax.set_facecolor('#000000')

        # plot historical tail (last 30 days)
        ax.plot(past_dates, hist_vol, color="#666666", linewidth=2, label="HISTORICAL VOLATILITY (30D)", zorder=2)

        # connect the historical tail to the median forecast
        connector_dates = [past_dates[-1], future_dates[0]]
        connector_p50 = [hist_vol[-1], p50[0]]
        ax.plot(connector_dates, connector_p50, color="#00e5ff", linewidth=2, linestyle="--", zorder=3)

        # plot forecast cone
        ax.plot(future_dates, p50, label="P50 MEDIAN FORECAST", color="#00e5ff", linewidth=3, zorder=4)
        ax.fill_between(future_dates, p10, p90, color="#00e5ff", alpha=0.1, label="80% CONFIDENCE INTERVAL", zorder=1)
        ax.axhline(y=p50[0], color="#333333", linestyle="--", linewidth=1.5, zorder=0, label="T=0 BASELINE")

        ax.set_title(f"[{ticker}] VOLATILITY FORECAST & TERMINAL DATA", fontsize=16, pad=20, loc='left',
                     color="#ffffff")
        ax.set_xlabel("MARKET TIMELINE", fontsize=10, color="#666666")
        ax.set_ylabel("PROJECTED VOLATILITY (%)", fontsize=10, color="#666666")

        # format x-axis with real calendar dates
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%b %d'))
        fig.autofmt_xdate(rotation=0, ha='center')

        # sharp terminal grid and spines
        for spine in ['top', 'right']: ax.spines[spine].set_visible(False)
        for spine in ['left', 'bottom']: ax.spines[spine].set_color('#333333')
        ax.tick_params(colors='#666666')
        ax.grid(True, color='#1a1a1a', linestyle='-', alpha=0.8)

        legend = ax.legend(loc="upper left", fontsize=9, facecolor='#000000', edgecolor='#333333', framealpha=1)
        for text in legend.get_texts(): text.set_color('#999999')

        # metrics panel with sharp borders (standardized to 30 chars, size 11)
        pe_str = f"{pe_ratio:.2f}" if isinstance(pe_ratio, (int, float)) else pe_ratio
        metrics_text = (
            f"TERMINAL METRICS\n"
            f"------------------------------\n"
            f"LAST PRICE : ${current_price:.2f}\n"
            f"AVG COST   : {avg_cost_str}\n"
            f"PROFIT/LOSS: {pnl_str}\n"
            f"TARGET     : ${target_price:.2f}\n"
            f"P/E RATIO  : {pe_str}\n"
            f"RSI (14D)  : {current_rsi:.1f}\n"
            f"52W HIGH   : ${financials.get('fiftyTwoWeekHigh', 'N/A')}\n"
            f"52W LOW    : ${financials.get('fiftyTwoWeekLow', 'N/A')}"
        )
        metrics_text = "\n".join([line.ljust(30) for line in metrics_text.split('\n')])
        ax.text(1.15, 0.95, metrics_text, transform=ax.transAxes, fontsize=11, verticalalignment='top',
                bbox=dict(boxstyle='square,pad=1.2', facecolor='#000000', edgecolor='#333333', linewidth=1),
                color='#cccccc')

        # sharp signal badge (standardized to 30 chars, size 11)
        signal_text = f"ACTION: {signal}"
        signal_text = "\n".join([line.ljust(30) for line in signal_text.split('\n')])
        ax.text(1.15, 0.45, signal_text, transform=ax.transAxes, fontsize=11, verticalalignment='center',
                bbox=dict(boxstyle='square,pad=1.2', facecolor='#000000', edgecolor=color, linewidth=1.5),
                fontweight='bold', color=color)

        # sharp logic panel (standardized to 30 chars, size 11)
        wrapped_reasoning = "\n".join(textwrap.wrap(reasoning, width=30))
        logic_text = f"SYSTEM LOGIC\n------------------------------\n{wrapped_reasoning}"
        logic_text = "\n".join([line.ljust(30) for line in logic_text.split('\n')])

        ax.text(1.15, 0.35, logic_text, transform=ax.transAxes, fontsize=11,
                verticalalignment='top',
                bbox=dict(boxstyle='square,pad=1.2', facecolor='#000000', edgecolor='#333333', linewidth=1),
                color='#999999')

        # enforce a true two column canvas layout with padding
        plt.subplots_adjust(left=0.05, right=0.55)

        save_path = f"{ticker}_terminal_forecast.png"
        plt.savefig(save_path, bbox_inches='tight', facecolor=fig.get_facecolor(), edgecolor='none', pad_inches=0.3)
        print(f"[success] output saved: {save_path}")


def fetch_and_format_live_data(ticker: str):
    print(f"[system] fetching live market data: {ticker}")
    end_date = dt.datetime.now()
    start_date = end_date - dt.timedelta(days=100)

    financials = yf.Ticker(ticker).info
    pipeline = marketdatacollector(tickers=[ticker], start_date=start_date.strftime("%Y-%m-%d"),
                                   end_date=end_date.strftime("%Y-%m-%d"))
    df = pipeline.fetch_data()

    # format index to extract clean plotting dates
    df.index = pd.to_datetime(df.index)

    # grab raw scaling parameters to mathematically reverse the normalization later
    vol_min = df['volatility_7d'].min()
    vol_max = df['volatility_7d'].max()

    df_scaled = pipeline.scale_features(df, ['Close', 'Volume', 'returns', 'volatility_7d', 'rsi_14'])

    # isolate the historical tail (using raw data to bypass the scaler entirely for plotting)
    recent_30 = df_scaled.tail(30)
    recent_30_raw = df.tail(30)

    past_tensor = torch.tensor(recent_30[['Close', 'Volume', 'rsi_14']].values, dtype=torch.float32).unsqueeze(0)
    past_dates = recent_30.index.tolist()
    hist_vol = recent_30_raw['volatility_7d'].values * 100  # convert to real percentage

    # isolate the future timeline
    future_dates = [end_date + dt.timedelta(days=i) for i in range(1, 15)]
    future_tensor = torch.tensor([[d.weekday(), d.month] for d in future_dates], dtype=torch.float32).unsqueeze(0)

    return torch.zeros((1, 1), dtype=torch.float32), past_tensor, future_tensor, financials, df.tail(
        1), past_dates, future_dates, hist_vol, vol_min, vol_max


if __name__ == "__main__":
    print("\n=== STOCKY ===")

    # user input handling for ticker and purchase price
    target_ticker = input("ticker symbol: ").strip().upper()
    if not target_ticker:
        print("[info] empty input. defaulting to AAPL.")
        target_ticker = "AAPL"

    purchase_input = input(f"average cost [{target_ticker}] (leave blank if none): ").strip()
    purchase_price = float(purchase_input) if purchase_input else None

    try:
        engine = liveforecaster()
        static, past, future, financials, recent_data, past_dates, future_dates, hist_vol, vol_min, vol_max = fetch_and_format_live_data(
            target_ticker)
        engine.analyze_and_plot(target_ticker, static, past, future, financials, recent_data, purchase_price,
                                past_dates, future_dates, hist_vol, vol_min, vol_max)

    except ValueError:
        print("[error] invalid cost basis provided.")
    except FileNotFoundError:
        print("[error] checkpoints/best_tft_model.pt not found. execute main.py to train.")
    except Exception as e:
        print(f"[error] execution failed for {target_ticker}: {e}")